<template>
	<view>
		<view class="body_gong" v-for="(o,index) in list" :key="index" >
			<view class="title"  @click="xq(o.id)">{{o.lx==1?"个人困难":"纠纷调解"}} 
				<text class="zhaungtai" :class="o.status==1?'color2':'color1'">
					{{o.status===1 ? "已安排负责人" :"待处理"}}
				</text>
			</view>
...
export default {
		...
		methods: {
			gethome() {
				uni.$u.http.post('SeekHelp/xcxlist', {wxid:uni.getStorageSync('userData').uid,page:this.page,limit:10}).then(res => {
			...
			xq(id){
				uni.$u.route({
					url: "/pages/componentsA/bangzhu/details?id="+id
				})
