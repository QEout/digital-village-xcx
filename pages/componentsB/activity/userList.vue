<template>
	<view>
		...
		<view class="biankuang" v-for="(itme,index) in list " @click="JOM(itme.id)" :key="index">
			<image :src="itme.ioc" mode="widthFix" class="huodongimage"></image>
			<view class="huodongtitle">{{itme.title}}</view>
			<view class="huodongtitle"><text>报名状态：{{itme.zid==0?"审核中": itme.zid==2?"报名失败,如有疑问请联系！":"已通过"}}</text></view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			JOM(id){
				uni.$u.route({
					url: "/pages/componentsB/activity/details?id="+id+"&showBaoMing=false"
				})
			}
