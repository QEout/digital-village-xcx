<template>
	<view>
		<view class="body_gong" v-for="(o,index) in list" :key="index" >
			...
			<!-- {{o.lx==1?"投诉":"建议"}} -->
				<text class="zhaungtai" :class="o.status==0?'color1':'color2'">
					{{o.status===1 ? "已接收" :o.status===2 ? "已处理" :"待接收"}}
				</text>
			</view>
...
export default {
		...
		methods: {
			gethome() {
				uni.$u.http.post('Suggestion/xcxlist', {wxid:uni.getStorageSync('userData').uid,page:this.page,limit:10}).then(res => {
			...
			xq(id){
				uni.$u.route({
					url: "/pages/componentsA/suggestion/details?id="+id
				})
